...
export function RepNoShowChatPanel({ periodDays }: { periodDays: number }) {
  ...
  async function sendMessage(rawMessage: string) {
    ...
    try {
      const response = await fetch("/api/rep-no-show-chat", {
        method: "POST",
        headers: {
  ...
  return (
    <Sheet>
      <SheetTrigger
        render={
          <Button
            ...
            aria-label="Ask Magic Mike about rep no-shows"
          >
      ...
      <SheetContent
        ...
        aria-describedby="rep-no-show-chat-description"
      >
        ...
        <div ref={messagesRef} className="flex-1 space-y-4 overflow-y-auto bg-muted/20 p-5">
          ...
          {isSending ? (
            <div className="flex max-w-[92%] items-center gap-2 rounded-2xl border bg-card px-4 py-3 text-sm text-muted-foreground shadow-xs">
              ...
              Reading the latest no-show stats...
            </div>
          ) : null}
        ...
        <form onSubmit={submitMessage} className="border-t bg-background p-4">
          <div className="flex items-end gap-2">
            <Textarea
              ...
              placeholder="Ask about rep no-shows..."
